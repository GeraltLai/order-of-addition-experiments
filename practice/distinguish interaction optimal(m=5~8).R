...
data=read.table(file="C:/Users/stupi/Desktop/study/order of addition/Tsai_2021_JSPI/catalogue/optimal/8/optimal_8_48.txt",header=F)
...
dis2=function(D){
  if(ncol(D)==5){
    ...
    FF=NULL
    for(k in 1:(ncol(D)-2))
      for(kk in (k+1):(ncol(D)-1))
        for(kkk in (k+2):ncol(D)){
          AA=as.matrix(D)
          for(i in 1:nrow(AA))
            for(j in 1:ncol(AA)){
              if(AA[i,j]==k){
    ...
    FF=NULL
    for(k in 1:(ncol(D)-3))
      for(kk in (k+1):(ncol(D)-2))
        for(kkk in (k+2):(ncol(D)-1))
          for(kkkk in (k+3):ncol(D)){
            AA=as.matrix(D)
            for(i in 1:nrow(AA))
              for(j in 1:ncol(AA)){
                if(AA[i,j]==k){
